...
def norm_digit(img):
    ...
    h, w = img.shape[:2]
    aff = np.array([[1, 0, w/2 - cx], [0, 1, h/2 - cy]], dtype=np.float32) # 가운데로 이동한 좌표
    dst = cv2.warpAffine(img, aff, (0, 0)) # 회전 변환
    return dst
...
desc = []
for img in cells:
    img = norm_digit(img) # 무게중심 이용하여 draw image center로 위치 변환
